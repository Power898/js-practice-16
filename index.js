...
{ console.error("Якщо ви бачите це повідомлення, то завдання 1 виконано не правильно"); console.clear(); console.debug("Завдання: 1 =============================="); const result = students.filter(x => {return x.grade === grade});
 ...
 */
function logArrayElements(arr){ if (!Array.isArray(arr)) {
...
function warnAboutMaxArrayLength(arr, maxLength) {
  if(!Array.isArray(arr),typeof maxLength !== "number"){ console.error("Перший аргумент має бути масивом, другий аргумент має бути числом!");} if(arr.length> maxLength) { console.warn("Увага! Довжина масиву перевищує максимально допустиму!");}
...
function compareArrays(arr1, arr2) {
  arr1.sort((a,b) => a[1]-b[1]); arr2.sort((a,b) => a[1]-b[1]); 
  const rez1= JSON.stringify(arr1 ); const rez2= JSON.stringify(arr2); if(rez1 !==rez2) { console.assert();}
...
function buildMatrix(size) {
  if(typeof size !=="number") {console.error("Аргумент має бути числом!"); return null;} 
  let matrix =[];  for (let i = 0; i <size; i++){ let arrS = []; for(let j=0; j<size; j++){ arrS[j] = Math.abs(Math.ceil(Math.random() * 10 - 1));}
    matrix[i] = arrS;
  }
 ...
 *  n - стартове число.
 */
function traceBackward(n) { console.log(n); if( n>1){ traceBackward(n-1); } else {console.trace();}
 ...
 */
function displayGroupedInfo(students) {for( let student of students){ console.group(`Студент: ${student.name}`); 
console.info(`Вік: ${student.age}`); console.info(`Предмет: ${student.subject}`);console.groupEnd();}
 ...
 */
function validateUserInput(userInput) {if (!userInput.hasOwnProperty('username')) {
  console.error(`Помилка: ім'я користувача відсутнє!`);} if(!userInput.hasOwnProperty('password')) {
  console.error(`Помилка: пароль відсутній!`);} if(!userInput.password.length < 8) {
...
function calculateTotalPrice(products) {
  console.time('myTimer'); let total =0; products.forEach(x=> {total += x.price;}); console.log(`Загальна вартість товарів: ${total}`);console.timeEnd('myTimer');
 ...
 */
function countVowelsAndConsonants(word) {let string ="aeiou"; for(let item of word){ let lowerItem = item.toLowerCase();if (string.includes(lowerItem)) { console.count(`Голосна`); } else { console.count(`Приголосна`); } console.countReset(`Голосна`); console.countReset(`Приголосна`);}
